...
function RouteLabel({ midpoint, walkingText, drivingText, distanceText }: RouteLabelProps) {
  ...
  const icon = L.divIcon({
    ...
    ">
      ...
      ${distanceText ? `<span style="opacity:0.3">|</span><span>${distanceText}</span>` : ''}
    </div>`,
    iconSize: [0, 0],
    iconAnchor: [0, 0],
  })
      ...
      {route && route.length > 1 && (
        <>
          ...
          {routeSegments.map((seg, i) => (
            <RouteLabel key={i} midpoint={seg.mid} walkingText={seg.walkingText} drivingText={seg.drivingText} distanceText={seg.distanceText} />
          ))}
        </>
      )}
    </MapContainer>
